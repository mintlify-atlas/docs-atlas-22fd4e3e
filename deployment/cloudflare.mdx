---
title: "Deploy nano to Cloudflare Workers and Pages"
...
Cloudflare Workers and Pages offer globally distributed, low-latency hosting for nano. When nano detects that it is being built for Cloudflare, it automatically switches to the `@astrojs/cloudflare` adapter, compiling the app into a `_worker.js` bundle that runs natively in the Workers runtime. The repository ships a `wrangler.toml` that pre-configures the deployment.
...
<Steps>
  ...
  <Step title="Deploy with Wrangler">
    ...
    Wrangler uploads your `dist/` directory to Cloudflare Pages. On first run, Wrangler asks you to authenticate and select or create a Pages project. Subsequent deploys update the same project.
  </Step>
</Steps>
