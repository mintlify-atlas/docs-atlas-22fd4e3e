...
sidebarTitle: "/api/scrape"
description: "Resolve a playable stream URL for a movie or TV episode by TMDB ID using an installed scraper plugin. Returns proxied stream URL, HLS flag, subtitles, and quality options."
---
...
<ParamField query="provider" type="string">
  The scraper plugin ID to use (e.g. `"rei"`). The provider must be present in the configured server allow-list. If it is not, the request falls back to the configured `DEFAULT_SERVER`. When omitted, the default server is used directly.
</ParamField>
