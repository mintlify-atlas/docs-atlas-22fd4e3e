---
title: "nano scraper plugin system: installing and managing sources"
sidebarTitle: "Plugins"
description: "nano uses encrypted .rink plugin files to resolve stream URLs. Install bundled sources with the shiopa CLI or create custom plugins that implement the ScraperPlugin interface."
---

nano's stream resolution is powered by a plugin system. Each plugin is a `.rink` file — an AES-256-CBC encrypted JavaScript module that exports an object implementing the `ScraperPlugin` interface. Plugins live in `src/shade/private/` and are loaded at build time by Vite's glob import, decrypted in memory, and executed at runtime. No plugin source code is ever exposed to the client.

## How plugins work

<Steps>
  <Step title="Glob import at build time">
    Vite loads all `.rink` files from both `src/shade/catalog/` and `src/shade/private/` as `Uint8Array` binary data using `import.meta.glob` with the `?uint8array` query. The catalog directory contains bundled pre-compiled sources; the private directory is where activated plugins live.
  </Step>
